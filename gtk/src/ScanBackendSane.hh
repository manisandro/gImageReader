...
private:
	...
	std::vector<uint8_t> m_lineBuffer;	// Buffer for received line
...
std::vector<ScanBackend::Device> ScanBackendSane::detectDevices()
{
...
bool ScanBackendSane::openDevice(const std::string& device)
{
...
bool ScanBackendSane::getParameters()
{
	...
	g_debug("Parameters: format=%s last_frame=%s bytes_per_line=%d pixels_per_line=%d lines=%d depth=%d",
			getFrameModeString(m_parameters.format).c_str(),
...
ScanBackendSane::ReadStatus ScanBackendSane::read()
{
	...
	if(m_nUsed >= m_lineBuffer.size()){
		g_assert(m_nUsed == m_parameters.bytes_per_line);
		++m_lineCount;
...
{
	Gdk::Pixbuf::create_from_data(m_imgbuf.data(), Gdk::COLORSPACE_RGB, false, 8, m_rowstride/3, m_height, m_rowstride)->save(job->filename, "png");
	m_imgbuf.clear();
...
const SANE_Option_Descriptor* ScanBackendSane::getOptionByName(const std::map<std::string, int>& options, SANE_Handle, const std::string& name, int &index)
{
...
bool ScanBackendSane::setStringOption(SANE_Handle handle, const SANE_Option_Descriptor *option, SANE_Int option_index, const std::string& value, std::string* result)
{
...
bool ScanBackendSane::setConstrainedStringOption(SANE_Handle handle, const SANE_Option_Descriptor *option, SANE_Int option_index, const std::vector<std::string>& values, std::string* result)
{
	if(option->type != SANE_TYPE_STRING){
		return false;
	}
...
Glib::ustring ScanBackendSane::getFrameModeString(SANE_Frame frame)
{
