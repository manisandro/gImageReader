...
- You can manage the list of opened images with the buttons next to the open button ![](https://raw.githubusercontent.com/manisandro/gImageReader/master/packaging/win32/skel/share/icons/hicolor/22x22/actions/list-remove.png)   ![](https://raw.githubusercontent.com/manisandro/gImageReader/master/packaging/win32/skel/share/icons/hicolor/22x22/places/user-trash.png)   ![](https://raw.githubusercontent.com/manisandro/gImageReader/master/packaging/win32/skel/share/icons/hicolor/22x22/actions/edit-clear.png). 
  Temporary files (such as screenshots and clipboard  data) are automatically deleted when the program exits.
...
- Multiple images can be selected, which lets the user process multiple images in one go (see below).

## Preparing for recognition

- To perform OCR on an image, specify:        
...
- The **input images** correspond to the selected entries in the *images tab* in the *sources pane*. If multiple images are selected, the program will treat the set of images as a multipage document and ask the user which pages to process when recognition is started.
...
- In hOCR mode, the OCR engine recognizes entire page of the selected source(s). There is no option to recognize individual selections. 
...
- Adjacent word items can be merged by right-clicking the respective selected items.
